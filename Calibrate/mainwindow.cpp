...
void MainWindow::notify(const Service::serviceID_t &service, const Service::State &st)
{
    ...
    case Services::CoefsOut:
        ...
        case Service::State::ReceiveComplete:
            ui->textLog->appendPlainText("Выполнен приём выходных коэффициентов");
            break;
        case Service::State::TransmitComplete:
            ui->textLog->appendPlainText("Выполнена передача выходных коэффициентов");
            break;
        }
        break;
    case Services::CoefsIn1:
        switch (st) {
        case Service::State::ReceiveComplete:
            ui->textLog->appendPlainText("Выполнен приём входных коэффициентов по каналу 1");
            break;
        case Service::State::TransmitComplete:
            ui->textLog->appendPlainText("Выполнена передача входных коэффициентов по каналу 1");
            break;
        }
        break;
    case Services::CoefsIn2:
        switch (st) {
        case Service::State::ReceiveComplete:
            ui->textLog->appendPlainText("Выполнен приём входных коэффициентов по каналу 2");
            break;
        case Service::State::TransmitComplete:
            ui->textLog->appendPlainText("Выполнена передача входных коэффициентов по каналу 2");
            break;
        }
        break;
...
void MainWindow::on_btnConnect_clicked()
{
    try {
        ...
        if (port->setBaudRate(7372800) && port->setDataBits(QSerialPort::Data8) && port->setParity(QSerialPort::NoParity) && port->setStopBits(QSerialPort::OneStop) && port->setFlowControl(QSerialPort::HardwareControl)) {
...
void MainWindow::showSetting(void)
{
    if (out->exec() == QDialog::Accepted) {
        ...
        out->transmit(1, Service::Type::TransmitStreaming);
            ...
            ui->textLog->appendPlainText("Порт не доступен!");
        }
    }
}
...
void MainWindow::calIn(std::shared_ptr<coefInService> coefsIn, char channel, float min, float max, float phase)
{
    ...
    case processIn_x1_8:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x1/8: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_1_8_a).arg(coefsIn->Volt_1_8_b).arg(coefsIn->Phase_1_8));
        break;
    case processIn_x1_4:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x1/4: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_1_4_a).arg(coefsIn->Volt_1_4_b).arg(coefsIn->Phase_1_4));
        break;
    case processIn_x1_2:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x1/2: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_1_2_a).arg(coefsIn->Volt_1_2_b).arg(coefsIn->Phase_1_2));
        break;
    case processIn_x001:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x1: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_001_a).arg(coefsIn->Volt_001_b).arg(coefsIn->Phase_001));
        break;
    case processIn_x002:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x2: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_002_a).arg(coefsIn->Volt_002_b).arg(coefsIn->Phase_002));
        break;
    case processIn_x004:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x4: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_004_a).arg(coefsIn->Volt_004_b).arg(coefsIn->Phase_004));
        break;
    case processIn_x008:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x8: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_008_a).arg(coefsIn->Volt_008_b).arg(coefsIn->Phase_008));
        break;
    case processIn_x016:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x16: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_016_a).arg(coefsIn->Volt_016_b).arg(coefsIn->Phase_016));
        break;
    case processIn_x032:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x32: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_032_a).arg(coefsIn->Volt_032_b).arg(coefsIn->Phase_032));
        break;
    case processIn_x064:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x64: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_064_a).arg(coefsIn->Volt_064_b).arg(coefsIn->Phase_064));
        break;
    case processIn_x128:
        ...
        ui->textLog->appendPlainText(QString("Расчитаны коэф. канала %1 для входного усиления x128: a=%2; b=%3; phase=%4").arg(channel).arg(coefsIn->Volt_128_a).arg(coefsIn->Volt_128_b).arg(coefsIn->Phase_128));
        break;
    }
}
...
void MainWindow::on_btnCalOut_clicked()
{
    ...
    case processOut_x1:
        ...
        ui->textLog->insertPlainText(QString("Расчитаны коэф. для выходного усиления x1: a=%1; b=%2;").arg(coefsOut->Out_x1_a).arg(coefsOut->Out_x1_b));
        break;
    case processOut_x2:
        ...
        ui->textLog->insertPlainText(QString("Расчитаны коэф. для выходного усиления x2: a=%1; b=%2;").arg(coefsOut->Out_x2_a).arg(coefsOut->Out_x2_b));
        break;
    case processOut_x4:
        ...
        ui->textLog->insertPlainText(QString("Расчитаны коэф. для выходного усиления x4: a=%1; b=%2;").arg(coefsOut->Out_x4_a).arg(coefsOut->Out_x4_b));
        break;
    case processOut_x8:
        ...
        ui->textLog->insertPlainText(QString("Расчитаны коэф. для выходного усиления x8: a=%1; b=%2;").arg(coefsOut->Out_x8_a).arg(coefsOut->Out_x8_b));
        break;
    }
}
